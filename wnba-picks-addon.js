...
(function () {
  ...
  var LGS = ['wnba'];   // 籃球板：WNBA 單檔（2026-08-03 由棒球 expert-picks-addon.js 整份複製,只換聯盟與資料源）
  ...
  var ALIAS = {};   // 籃球隊名無異體字問題（棒球的 華老鷹→韓華鷹 不適用）
  function tmEq(a, b) {
  ...
  // 籃球頁不自動包裝 renderCard：nba.html 於每張卡 append 後直接呼叫 __expertPicks.decorate(model)
  ...
  function fetchFeed() {
    return Promise.all([
      ...
      Promise.resolve(null),   // 籃球無 legacy 單檔
    ]).then(function (rr) {
  ...
  window.__expertPicks = { decorate: decorate, picksForCard: picksForCard, aggregate: aggregate, optOf: optOf, weightOf: weightOf, epStrong: epStrong, hasNew: hasNewFor, _setData: function (d) { data = d; }, _mergeFeeds: _mergeFeeds, _mergeArchives: _mergeArchives, _fmtUpd: fmtUpd,
