/* ============================================================
   ...
   每次執行：抓 MLB 今明賽程 → 對「開打前 55–120 分鐘、未起卦」的比賽，
   ...
   排程：divination_lab/liuyao-cast.yml.example（複製到 .github/workflows/ 才會啟用）
   ============================================================ */
'use strict';
const fs = require('fs');
...
(async function main() {
  ...
  const inWindow = games.filter(g => { const m = (g.ts - now) / 60000; return m >= 55 && m <= 120; });
  ...
  console.log(`賽程 ${games.length} 場｜視窗內 ${inWindow.length}｜待起卦 ${todo.length}${DRY ? '（dry）' : ''}`);
  ...
  if (DRY) { pulse = { outputValue: crypto.randomBytes(64).toString('hex'), timeStamp: 'DRY', uri: 'DRY' }; }
    ...
    catch (e) { const fail = { failedAt: new Date().toISOString(), reason: 'beacon:' + e.message, gamePks: todo.map(g => g.gamePk) }; ledger.push(fail); fs.writeFileSync(LEDGER, JSON.stringify(ledger, null, 1)); console.error('beacon 失敗，已留痕，下輪重試'); return; }
  }
  const t = twNow();
