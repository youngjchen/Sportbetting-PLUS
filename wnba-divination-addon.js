...
(function () {
  ...
  async function fetchGames() {
    try {
      ...
      const arr = (j && j.games) || [];
      const now = Date.now();
      games = arr.filter(g => g.league === 'WNBA' && g.status !== 'finished' && g.date && g.time)
        .map(g => ({ ...g, awayTeam: g.away, homeTeam: g.home,
          lotteryTotal: g.totLine != null ? g.totLine : null,
          lotteryHandicap: (g.hdFav && g.hdVal != null) ? { favSide: g.hdFav, line: g.hdVal } : null,
          ts: Date.parse(g.date + 'T' + g.time + ':00+08:00') }))
  ...
  async function doCast() {
    ...
    try {
      ...
      // 起卦即排程雲端備份（90 秒防抖；沒設權杖會安靜略過）——手動卦從此不再只活在這台瀏覽器
      try { if (window.__dvSync) window.__dvSync.schedule(); } catch (_) {}
