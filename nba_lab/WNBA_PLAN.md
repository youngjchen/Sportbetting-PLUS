...
- ✅ 階段0 全過：join 220/222、teamA=主場、負=客讓（台彩同側99.5%）、隊名對照15隊零衝突 → WNBA_AUDIT_REPORT.md
- ✅ 階段1 全過：種子219場、**對帳4隊×4市場×主客全=玩運彩隊伍頁逐格一致**；nba.html 聯盟化+統計頁+卡面過盤率（A案定稿+主客｜綜切換,比分/隊名置中修正）
- ✅ 階段2 程式碼+本機驗收：wnba_scraper.js（今9場含LIVE節數/明日帶盤）、wnba_titan_odds.js（7/7場配對,bet365藍帶+線移/顛倒偵測上卡）、排卡/燈注互動/自動結算迴路 preview 實測綠
- ✅ 階段3 程式碼：ALLIANCES+{7,wnba}、D11映射（test_wnba_env 守門）、鬧鐘 wnba（hot 12/18/22,deep 04:40）、expert-picks-wnba.yml、板端明牌膠囊+面板；loadPrev 冷啟動分支（新聯盟檔不存在=合法起步,毒檔防護不變）
- ⏳ 雲端驗收：兩條 workflow 首航＋隔日 watchdog 時窗行為＋明牌首夜波表（部署後觀察）
- 📌 D11/深掃04:40/台彩主軸/A案 全獲使用者 2026-08-03 核准（「全同意」+棒球卡截圖定版式）
